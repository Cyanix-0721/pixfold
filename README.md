...
> 阶段归属说明：**SAF 可行性 spike 属 §8 技术栈决策流程**（回答"能不能做"，用于排除证伪条件 ①②，已于 2026-09-08 提前完成 5/5），**不属于 D2 阶段**；D2 回答的是"做成什么样"（产品级平台能力）。spike 工程与 D1 原型工程均已入库 `ready` 分支（验证阶段资产，结束后按需迁移或清理）。

> 进度(2026-09-08):D0 已完成;Android SAF spike 按 HANGOFF §8.2 提前执行并 **5/5 验收**(属 §8 技术栈决策),证伪条件 ①② 排除,**Flutter + Dart 已锁定**;当前主线为 D1 交互原型(不连真实文件系统)。执行顺序与判定以 HANGOFF §8.2 / §8.3 / §9 为准。
> 进度(2026-09-09):**D1 交互原型已启动**——原型工程 `C:\Personal\pixfold\pixfold-d1`(确定性 mock 数据,Windows + Android 双端验收)。Windows 端首轮走查反馈已闭环:预览自适应+缩放拖动、桌面即时拖拽、中文发虚(禁 Impeller + 显式微软雅黑)、网格拖拽黑边(详见 HANGOFF §9 D1 执行注 / §12.5)。
> 进度(2026-09-10):**拖拽定案自研**(三方包 drop 不落地 → 弃用,原型**零三方依赖**);修复"数据变了界面不刷新"(通知链断裂根因,详见 HANGOFF §12.5)。**流程优化**:§9 新增《D1 验收清单》(20 项,双端逐条打勾)+ 阶段门槛表(每阶段可判定退出条件)+ **决策门**(清单全绿才创建正式工程;换栈触发=§8.2 证伪条件 ③,已实证排除)+ "修复自证三层"约定。**归属修正**:SAF spike 归 §8(不再并入 D2);**两个验证工程入库 ready 分支**。**走查状态:Windows 全量走查进行中,Android 侧尚未测试**;D1 验收清单走查通过前不宣称完成。

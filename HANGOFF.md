...
> **属"拿语义换染色"的取舍失当**。
> **正解**:自绘一个**可染色的图钉**(Canvas),同时满足语义正确 / 可染色 / 零三方依赖
> (不引入 `material-icons`)。已抽为共享 `PinToggle`(原先卡片与列表行各写一份、且各用星号)。
> **两态不只靠颜色**:未固定 = 钉帽**描边** + 两条斜边;已固定 = 钉帽 + 钉身**实心**,
> 单色/弱视场景亦可区分。
> **防回退护栏**:`PinButtonTest` 含"不得再出现 ★/☆"断言;变异验证 = 改回星号即失败。

> **真机核对证据(`192.168.43.1:4444`,2026-09-17)**:将截图按图钉 `bounds` 裁切后**逐像素转 ASCII** 判形
> (截图附件在本环境不可直接读,故用像素图案代替肉眼看图,判据更客观):
> - **未固定态**:空心**图钉轮廓** —— 圆钉帽 + 收窄钉身 + 尖端,非五角星;
> - **已固定态**(点按后描述变 `已固定`):**实心图钉剪影**,底色为 `primary`;
> - **列表视图**同款实心图钉,尺寸 168px = **48dp**,满足 M3 触控下限;
> - 页面上**已无 ★/☆ 字形**(控件树中 `★`/`☆` 检索为空);**无崩溃**。

**W2 实现记录(2026-09-17)**:滚动位置由 `LazyGridState` / `LazyListState` 驱动;
可见性判据 = `canScrollForward`(下方还有内容)**且** `firstVisibleItemIndex > 0`(确实下滑过),
以免"内容不足一屏"或"已回顶"时误显;点击 `animateScrollToItem(0)` 平滑回顶。
按钮为 M3 `FilledTonalIconButton` + **自绘向上箭头**(零三方依赖,不用 `material-icons`),
置于**右下角**(拇指易达,且不与底部系统手势条冲突),`contentDescription = "回到顶部"`。
**lint 抓到一处真实性能问题**:直接在 composition 里读 `firstVisibleItemIndex` 会**每像素滚动都重组**
(`FrequentlyChangingValue`),已按 lint 建议改用 `derivedStateOf` 过滤 —— 仅在"顶部/非顶部"翻转时重组。
**变异验证**:「按钮永远显示」被 3 项捕获、「点击不回顶」被 1 项捕获。新增 4 项断言。

...
| D2b 完整平台能力        | ⬜ 待办                 | **五项**:缩略图策略 / 后台任务与进度 / 授权失效恢复 / 存储空间预检与不足退路 / 大归档写入(Zip64·中断·文件系统上限),均须真机可复现且有失败路径提示(2026-09-19 校正:原文写"三项"但实列五项,系列举不全;§10《平台》已把后两项列为验收标准。Windows/Linux 验证项与 §7/§10"桌面无验收项"矛盾,已移出——见 D2b 节) |
| D3 领域核心与适配层     | 🔄 部分已由 D1 提前交付(2026-09-19 校正) | **排序/命名/元数据三类核心逻辑已随 D1 的 P1–P5 落地并单测通过**(`pixfold-d1/domain/`,`:domain:test` 143 项,不依赖 UI/平台);**剩余 = 打包核心 + 适配层抽取**,待 D4/D2b 落地后收口(见 D3 节重定义) |
...
>
> **⚠️ D1 ↔ D3 范围重叠的校正(2026-09-19 用户确认)**:上面这条执行注(2026-09-16 追加)把领域逻辑划进 D1,而 §9.1「切片原则」也要求"各阶段的领域逻辑随该阶段一起进" —— 两者叠加的结果是:**D3 退出条件里的"排序/命名/元数据"三类已由 D1 的 P1–P5 实际交付**(见 D3 节)。此前 D3 在门槛表上仍写"⬜ 待办",照字面读会导致 D1 收口后**重复实现已完成的领域核心**。按用户 2026-09-19 决定,D3 由"**重新实现领域核心**"改为"**验证与固化**"性质:不重复造已落地的领域层,只做剩余部分(打包核心 + 适配层抽取)与"正式工程可承接"的确认。详见 D3 节。
...
> **⚠️ 截至 2026-09-19 进度:P1–P5 已交付,清单仍为 **11/20 项通过**(第 1–10、20 项)**。
> P3 的两处缺口(第 4 项列表拖拽、第 7 项后半批次/本组独立接入 UI)已于第二轮补齐并经真机核对,详见下方"P3 收口记录";
> P4 的命名结构/建议表/冲突警告(第 8、9、10 项)已完成核对(超长一类仅领域层证据,详见「P4 收口记录」的局限说明);
> **P5 的元数据/ComicInfo.xml/页码预览(第 11–14 项)本轮仅完成第 1+2 层自证(领域单测 + UI 断言),
> 故记为 🟡 而非 ✅** ——「修复自证三层」的第 3 层真机实证尚未执行(用户要求本轮跳过真机部分),
> 详见下方「P5 收口记录」的局限说明;真机核对通过后方可转 ✅ 并把进度改写为 15/20。**
...
| 4  | 列表拖拽真实改变顺序                                       | §9/§10      | ✅      | UI 断言 5 项(`DragReorderListTest`:拖第1行到第3行 / 拖动中不上报 / 拖到自己不报 / 向下拖 / 图钉可点)**真机已核对通过**(第二轮实测:列表视图拖拽后顺序确实改变) |
| 5  | 人工调整有可见标记,且可一键重置回自动排序                  | §4          | ✅      | 真机:拖后显示 `已手动调整 1 处`,重置按钮由禁用转可用 |
| 6  | 固定位置(图钉)项在重新应用排序后保持原位                   | §4          | ✅      | 真机:固定 `002` 后应用排序 → 固定项保位、其余重排、人工标记清零 |
| 7  | 多级排序规则可增删改;批次默认与"本组独立"例外并存          | §4          | ✅      | 前半:规则编辑器 1..4 级(增删改 + 每级独立升降序 + 领域单测);后半**已接入 UI 并经真机核对**:集合切换 chip 行 + `本组独立` Switch(`TAG_CUSTOM_RULE_TOGGLE`)+ 作用范围标签(`batchLabel`),`applyRuleToCollection`/`toggleCustomRule`/`effectiveRule` 均已接线(不再是死代码);UI 断言 8 项(`BatchRuleUiTest`),含"独立集合应用规则不影响其他集合" |
| 8  | 命名结构:组件启用/顺序/分隔符/大小写/补零/扩展名策略可调   | §4          | ✅      | 领域单测 + UI 断言(改任一项后**建议名真的变**)+ **真机核对通过**;组件可增删/停用/上下移、分隔符/大小写逐组件可调、根目录名(跟随集合或手输)、起始序号/补零位数/扩展名策略/自动清洗 |
| 9  | 命名建议表:原路径→新名称,可逐项覆盖,且改结构后覆盖不丢失   | §6.3        | ✅      | **真机实证**:把第 2 行覆盖为 `KEEPME旅行照片_002.jpg` 后**关闭「根目录名」组件**,未覆盖行由 `旅行照片_001.jpg` 变为 `001.jpg`,而覆盖行**逐字未变**;清空输入框 = 撤销覆盖(非改空名);机制:覆盖存为**独立于 scheme** 的 `Map<imageId,name>`,不靠 UI 同步 |
| 10 | 冲突与警告可见:重名、非法字符、大小写冲突、超长            | §9/§10      | ✅      | **真机核对**:重名(禁用序号后 `共 14 项 · 冲突 14`,行内「重名：有 14 项同名…」)、跨目录重名(`扫描件_page_01.png` ×2)、非法字符(`扫描件_封:面?.png` + 「非法字符：: ?」)、超长(领域层 180 边界,⚠️ 未在真机复现,见备注);警告**分级配色**(会导致跳过=error 色 / 仅警告=次级色)+「仅问题」筛选 |
| 11 | 逐卷元数据 title/series/writer/volume 可改,建议值带来源    | §4/§6.1     | 🟡      | 领域单测 + UI 断言(`MetadataPageTest` 15 项):六个字段 + 语言下拉全部可编辑、改后输入框文本真的变、**每个建议值与其来源同屏可见**、逐字段「恢复建议」可点且值真的回退、一键清理作者可用;**⚠️ 真机未核对(本轮跳过,见 P5 收口记录)** |
| 12 | 语言默认"未设置"必须人工确认;支持逐卷不同,写入/不写入可选 | §4          | 🟡      | 领域单测(`language defaults to Unset even when a suggestion exists`)+ UI 断言:默认值显示「未设置（需确认）」+ `errorText 语言默认未设置，需人工确认`、**建议 `ja` 可见但不被采纳**、下拉可选 zh/ja/其他/未知/**不写入标签**(独立选项)、切卷后语言各自保留;**⚠️ 真机未核对(本轮跳过)** |
| 13 | 批量设置不覆盖逐项例外(被手工改过的字段保持)               | §6.3        | 🟡      | 领域单测(跳过/强制/未勾选三态 + 生效计数)+ UI 断言:批量后提示 **`已设置到 4 卷（跳过 1 卷逐项例外）`**、被跳过的卷值逐字未变、`强制覆盖逐项例外` 开关可覆盖;**⚠️ 真机未核对(本轮跳过)** |
| 14 | ComicInfo.xml 预览与页码列表预览(含重编号)一致             | §9/§10      | 🟡      | 领域单测(XML 字段序/2 空格缩进/**空值自闭合空标签**/只转义 4 字符;页码 001 起、`>=100` 转 4 位、扩展名小写、跟随**人工调整后的页序**;`previewOf` 同源求值)+ UI 断言(两份预览同屏、项数=页序长度、切卷后两份**同时**跟随、自闭合空标签与 `zh`/`ja` 写入);**⚠️ 真机未核对(本轮跳过)** |
...
| 19 | 两条工作流均可从首页完整走通到结果页                       | §9          | ☐       | 待 P7。**P5 后工作流 B 已不再是占位页**:首页 → 漫画库 → 元数据 → 打包预览三段已可走通(UI 断言 + `BackNavigationTest` 覆盖层级返回);**仍缺"结果页"**(执行报告 + 撤销入口属 P6),故本项保持未完成 |
| 20 | 工程健康:静态检查零告警;测试全绿(含 UI 层断言)             | 流程约定    | ✅      | `lint` 零告警(`warningsAsErrors`,报告 0 issues);**P5 收口实况:领域 143 项 + UI 205 项全绿**;负向对照(`NegativeControlTest`)仍如期失败 |
...
| **P3** | 拖拽排序(网格+列表,松手落地)、图钉入口、排序规则编辑器(多级/批次+本组独立) ✅ **完成(2026-09-17,第二轮收口)** —— 验收第 3、4、5、6、7 项全部通过(第 4 项列表拖拽与第 7 项后半"本组独立接入 UI"已补齐并经真机核对) | 3, 4, 5, 6, 7 ✅ | 1+2+3 |
| **P4** | 命名结构编辑器、建议表(原路径→新名)、逐项覆盖、冲突与警告 ✅ **完成(2026-09-18)** —— 第 8、9 项与第 10 项的三类冲突经真机核对;超长一类仅领域层证据(见「P4 收口记录」局限) | 8, 9, 10 ✅ | 1+2+3 |
| **P5** | CBZ 元数据编辑器、ComicInfo.xml 预览、页码列表预览 🟡 **离线交付(2026-09-19)** —— 第 11–14 项完成**第 1+2 层**自证(领域单测 + UI 断言);**第 3 层真机实证本轮按用户要求跳过**(见「P5 收口记录」局限) | 11, 12, 13, 14 🟡 | 1+2 |
...
**P3 完成记录(2026-09-17 首轮交付)**:网格拖拽排序、图钉入口、人工调整标记与一键重置、多级排序规则编辑器交付。
**首轮自检发现两处缺口**(列表拖拽未实现、批次/本组独立未接入 UI),故当时**明确判定 P3 不计为完成** ——
该判定记录保留在下方"首轮缺口与教训",作为"不虚报完成度"的过程证据。
...
**P3 收口记录(2026-09-17 第二轮,✅ 完成)**:首轮两处缺口已全部补齐并经真机核对 ——

| 缺口 | 补齐内容 | 自证 |
| --- | --- | --- |
| **第 4 项 列表拖拽** | `DragReorderList` + `ThumbRow` 接入 `detectDragGesturesAfterLongPress`;沿用网格侧同一套"父容器按 `boundsInRoot` 判目标格 + 松手才落地"语义(两个视图行为一致,不各写一套) | 领域/UI 断言 5 项(`DragReorderListTest`);**真机核对通过** |
| **第 7 项后半 批次/本组独立** | 页面接入集合切换 chip 行 + `本组独立` Switch + 作用范围标签;`applyRuleToCollection` / `toggleCustomRule` / `effectiveRule` 全部接线(**不再是死代码**) | UI 断言 8 项(`BatchRuleUiTest`),含"独立集合应用规则**不影响**其他集合";**真机核对通过** |

**收口时验证实况**:`:domain:test` **48 项**、`:app:testDebugUnitTest` **110 项**、`assembleDebug` 成功、
`lintDebug` **零告警**(数字为本次提交时实况;后续阶段若增测会继续上升)。

> **首轮缺口与教训(2026-09-17 自检发现,保留为过程记录)**:
> 1. **第 4 项(列表拖拽)未实现**——`ThumbRow` 无任何拖拽处理,列表视图当时**只能点击、不能拖动排序**。
>    当初把"网格+列表拖拽"一并计入 P3 交付物,但实现只落在网格侧。
> 2. **第 7 项后半未接入 UI**——`WorkflowAState`(`batchRule` + `customRuleCollections`)
>    已实现并有 9 项领域单测,但**页面仍直接用 `PageOrderState`**,`app` 内**零引用** → 该领域代码当时是**死代码**;
>    "批次统一 + 逐项例外并存"在界面上**不可操作**。
> **教训(已固化为流程约定)**:这两处都是"领域层做完 + 单测全绿"造成的**完成感假象** ——
> 单测只证明了领域逻辑正确,**不证明它被界面用上**。完成判据必须包含
> "该能力在真机上可操作",而非"有单测通过"(与 §9"修复自证三层"第 2、3 层同源)。

**P4 收口记录(2026-09-18,✅ 完成)**:命名结构编辑器、建议表、逐项覆盖、冲突与警告交付,
验收第 8、9、10 项**已按"修复自证三层"完成核对**(第 8、9 项与第 10 项的三类冲突经**真机**核对;
第 10 项的**超长**一类仅有领域层证据,**未在真机复现**,详见下方局限说明)。

**验证实况**:`:domain:test` **108 项**、`:app:testDebugUnitTest` **150 项**、`assembleDebug` 成功、
`lintDebug` **零告警**(数字为收口时实况)。

**领域层**(`domain/naming`,随本阶段一起进):`buildProposals` 严格照 §6.1 九步;
`attachWarnings` 五类一次遍历;`buildRenamePlan`;覆盖/组件的纯函数入口。
关键语义:空组件**连分隔符一起丢**、首个组件不输出分隔符、目录序号按**目录名**(换序不变)、
序号取**当前页序下标**、清洗作用于**含扩展名的完整名**、
`Duplicate` 与 `CaseCollision` **互斥**、`IllegalChar` **仅在 sanitize=false 时**产生、
`TooLong` 阈值为 **> 180**(恰好 180 不算)、仅 Duplicate/IllegalChar 导致跳过。

**"改结构后覆盖不丢失"的机制(验收第 9 项)**:覆盖存为**独立于 scheme** 的
`Map<imageId, name>`,不放进 `NamingScheme` —— 故改组件/分隔符/补零只影响 `proposedName`,
覆盖项逐字不变。这是**数据结构的性质**,不靠 UI 小心同步。

**真机证据**:
- 覆盖:第 2 行改为 `KEEPME旅行照片_002.jpg` 后**关闭「根目录名」组件**,
  未覆盖行由 `旅行照片_001.jpg` → `001.jpg`,覆盖行**逐字未变**;
- 冲突:禁用「图片序号」→ `共 14 项 · 冲突 14`;启用「原文件名」→ 跨目录重名
  `扫描件_page_01.png` ×2 + 非法字符 `扫描件_封:面?.png`(清洗关时显示「非法字符：: ?」)。

**本轮实测修掉的 4 个真实缺陷(均为产品缺陷,非测试问题)**:

| # | 现象 | 根因 | 修法 |
| --- | --- | --- | --- |
| 1 | 部分控件点不到、"顺序可调"名不副实 | 组件卡片**单行横向滚动** -> 靠后的上移/删除按钮落在视口外 | 改**两行卡片**,屏幕宽度内全部可达 |
| 2 | "自动清洗"开关被挤出屏幕 | 说明写成**长文案**,占满剩余宽度,把 `weight` 压到 0 | 说明改由**圆形感叹号图标**承载(点击弹窗),weight 恢复吸收剩余空间 |
| 3 | 建议表高度近乎 0,第 2 行都显示不出 | 外层 `verticalScroll` + 内层 `LazyColumn` **高度竞争** | 整页改**单个 LazyColumn**(头部为 item) |
| 4 | 行内有「人工覆盖」警告,汇总却写「警告 0」 | `hasWarningOnly` 用**枚举白名单**,漏掉 `Overridden` | 改为**取反**表达(有警告且非冲突),新增类型自动计入 |

**用户 2026-09-18 追加的两项设计决定**:
1. **根目录名跟随集合** —— 此前命名结构整体是批次级,导致切到 `扫描件` 集合后建议名仍是
   `旅行照片_005.png`。改为:默认取**当前集合**的 `rootDirName`,用户显式改过则记为**逐项例外**
   且切集合不被冲掉(即 HANGOFF §2 约束 4「批次统一 + 逐项例外」)。
   用独立可空字段 `rootDirNameOverride` 表达"是否覆盖",而非把集合名写回 scheme
   (后者分不清"跟随的默认值"与"用户输入")。**组件/分隔符/补零/扩展名仍是批次级**。
2. **清洗说明改用圆形感叹号图标 + 点击展开**(见上表 #2)。

**顺带修掉的功能缺口(用户提问"支持预测性返回手势了吗"——此前不支持)**:
真机实测:在"步骤2 命名结构"按返回会**直接退出应用**,用户丢失当前位置;
项目此前**完全没有返回处理**。修复后层级为
`预览 → 工作流A(步骤2) → 工作流A(步骤1) → 首页 → 退出`;
`WorkflowASteps` 用**单个** `BackHandler` 按层级决策(而非"非首步才启用 + 靠外层兜底"),
使注册始终 enabled —— 系统据此知道"返回有去处",预测性返回才会走应用内后退。
Manifest 显式 `enableOnBackInvokedCallback="true"`(targetSdk 33+ 默认,写明意图);
因 minSdk=24 该属性会触发 lint `UnusedAttribute`,以 `tools:targetApi="33"` 标注。

> **⚠️ 如实记录的范围与局限**:
> 1. **超长(第 10 项的四类之一)未在真机复现** —— 真机上让 `baseName` 达到 181 字符需长文本输入,
>    操作成本高且 `adb input text` 易被输入法干扰。该项由**领域单测**钉死
>    (`too long triggers only above 180`、`chinese counts as one unit`),
>    **不计入真机已核对的范围**。其余三类(重名/跨目录重名/非法字符)均已真机复现。
> 2. **`fillType="evenOdd"` 的作用曾被我判断错** —— 原以为"漏了它就看不见感叹号",
>    **实测删掉后渲染完全一致**:挖空实际来自两个子路径**绕向相反**(nonZero 也会挖空)。
>    已改正注释;真正的失败模式是"感叹号子路径缺失/与圆同向",由
>    `ExclamationIconRenderTest` 的"圆内应有洞"断言 + 变异验证覆盖。
> 3. **`uiautomator dump` 里没有 ≠ 没渲染** —— 说明对话框的**正文**在 dump 中完全不存在
>    (只有标题与按钮),但截图显示渲染正常。仅凭 dump 断言"内容缺失"会得出错误结论;
>    关键视觉结论须结合**截图像素分析**(本轮即用几何确认图标为"实心圆 + 感叹号":
>    垂直中轴挖空成 3 段、水平中轴 2 段)。

**P5 交付记录(2026-09-19,🟡 离线完成 / 真机待补)**:工作流 B 的**漫画库 → 逐卷元数据 → ComicInfo.xml 与页码预览**
三段流程交付,覆盖验收第 11–14 项。**本轮按用户要求跳过真机测试部分**,故第 3 层未做,清单记 🟡 而非 ✅。

**验证实况**:`:domain:test` **143 项通过**(P4 为 108,新增 35)、`:app:testDebugUnitTest` **205 项通过**(P4 为 150,新增 55)、
`:app:assembleDebug` 成功、`:app:lintDebug` **零告警**(报告 0 issues)。

**领域层(新增 `domain/comic/` + `domain/workflowb/` + `model/Metadata.kt` + `mock/MockLibraryBData.kt`)**:

| 文件 | 职责与关键语义 |
| --- | --- |
| `model/Metadata.kt` | `Suggestion`/`LangChoice`/`MetaField`/`MetaValue`/`ComicVolume`;`pendingIssues` 固定序(语言未确认 → 卷号缺失);`groupVolumesBySeries` 按**当前** series 值分组 |
| `comic/MetaEdit.kt` | `comicVolumeOf`(初值构造:**语言恒 `Unset`**)、`setMetaValue`(记逐项例外)、`useSuggestion`(恢复建议**并清除例外标记**)、`batchApply`(返回**生效/跳过**计数)、`langIso`、`defaultCbzName` |
| `comic/ComicInfoXml.kt` | `ComicInfo.build`(字段序固定、2 空格缩进、**空值自闭合空标签**、只转义 `& < > "`)、`renumberPages`(001 起 / `>=100` 转 4 位 / 扩展名小写 / 跟随人工页序)、`previewOf`(**XML 与页码同源求值**,一致性成为结构性质) |
| `workflowb/WorkflowBState.kt` | 三步骤共享状态(含 `activeVolumeId`),批量设置为纯函数入口 |

**UI 层(新增 `app/ui/workflowb/`,4 个文件)**:`LibraryPage`(系列分组 + 系列头聚合"N 卷待确认" + included 勾选 + 建议与来源)、
`MetadataPage`(六字段 + 语言下拉 + 逐字段"恢复建议" + 一键清理作者 + 批量设置行)、`PackPreviewPage`(XML 原文预览 + 页码列表预览 + 切卷)、
`WorkflowBSteps`(M3 `TabRow` 三步骤容器,**层级返回**:步骤3→2→1→首页)。`PixFoldApp` 的 `PlaceholderPage("工作流 B · 待实现（P5）")` 已移除。

**本轮实测修掉的真实缺陷(均为产品缺陷,不是测试问题)**:

| # | 现象 | 根因 | 修法 |
| --- | --- | --- | --- |
| 1 | 卷列表的**建议来源看不到** | 卷行第二行 `maxLines = 1`,来源被省略号截断 —— 等于"建议没有来源",第 11 项名存实亡 | 改 `maxLines = 2`,并**加窄屏(360dp)回归用例**钉死(复刻 P4"控件够不到"的教训) |
| 2 | 输出冲突卷在元数据页**无任何提示** | 冲突标记只做在步骤 1 的卷列表,编辑页没做 | 卷头补 `输出冲突：…` 提示 |
| 3 | Mock 数据的 Title 含卷号 -> 默认 CBZ 名重复 | `Title` 取"内层目录名"(已含「第02卷」),而 §8.3 又拼一次卷号,得到 `进击的巨人 第02卷 第02卷.cbz` | Title 建议改取**系列层**标题,卷号由 volume 单独承载(与规格 §8.3 的拼接语义自洽);外传无卷号故保留「外传」字样 |
| 4 | **取消勾选的卷在打包预览页无任何提示** | 预览页完全不看 `included`;领域层 `includedVolumes` **零引用 = 死代码** —— 与 P3「领域做完界面没用上」同类 | 未勾选卷:卷头红字"本卷未勾选,本次不会打包" + 卷 chip 标「（未勾选）」+ 头部显示 **「本次将打包 N / M 卷」**(由 `includedVolumes` 求出,死代码转为活代码);补 4 项回归断言 |

**变异验证(4 组,全部被捕获 —— 证明新测试不是空转)**:

| 变异 | 结果 |
| --- | --- |
| 语言**采纳建议**值(破坏"默认未设置"硬语义) | 5 项领域单测失败 |
| 空值**省略标签**而非自闭合空标签 | 2 项领域单测失败(含 L11"不写入"用例) |
| 批量设置**无视逐项例外** | `L14 batch apply skips…` 失败 |
| 页码补零**不随页数自适应**(恒 3 位) | `L12 padding becomes four digits…` 失败 |
| UI:预览页**永远取第一卷**(忽视当前卷) | 3 项 UI 断言失败(含"切卷后预览真的变") |
| UI:批量结果**只提示"已应用"** | 4 项 UI 断言失败(生效/跳过计数) |

> **⚠️ 如实记录的范围与局限(本轮必读)**:
> 1. **真机实证(第 3 层)本轮未做** —— 用户明确要求"除真机测试部分"。故第 11–14 项**不满足**
>    §9"修复自证三层"的完成判据,清单状态为 **🟡**,进度仍记 **11/20**。
>    **不得**以"离线测试全绿"冒充这四项完成;真机核对通过后转 ✅ 并改写为 15/20。
> 2. 第 2 层 UI 断言的两个被测页面是**无状态**的(变更上报上层),测试必须像真实上层那样持有并回灌状态。
>    本轮初版测试脚手架传 `onLibraryChange = {}`,导致"点切卷没反应"——那是**脚手架造成的假象**,
>    与 P3 的"数据变了界面不动"同类。已在两个测试类中固化为带状态的最小持有器,并写入注释。
> 3. 卷行的 `Text` 位于 `clickable` 卷卡片内,**语义合并**后合并树上找不到其 testTag,
>    必须用 `useUnmergedTree = true` 读取。这是 Compose 语义合并的正常行为,不是缺陷(已注释说明)。
...
| W3 | **排序规则升降序交互 + 变动即应用**(用户 2026-09-17 指定) | ✅ 已通过真机核对 | 每级默认升序;**再次点击已选中的字段**即切换升/降序;用**实心三角**(正/倒)表示方向,**仅显示在选中字段名之后**;原行尾的"升序/降序"切换按钮**已移除**;**并去掉"应用排序"按钮 —— 换字段/翻方向/增删级均即时生效** |
| W4 | **排序规则变更后自动回顶** | ✅ 已通过真机核对(⚠️ 无本地回归护栏,见下) | 规则一变即回顶 —— 重排后"当前位置"失去意义(用户看到的是中段而非新首项)。实现:`scrollToTopSignal` 自增 -> `LazyGridState/LazyListState.requestScrollToItem(0)`(**在本次布局内结算**,不用 `LaunchedEffect`);**仅 signal 变化时触发**,无关重组不得把用户拽回顶部 |
...
> **性能实测与优化(2026-09-17,用户提问:"一个集合最大 3000 张、单张最大 50MB,立即重排是否有性能问题?")**:
>
> **结论:没有问题。** 两条原因:
> 1. **排序只读元数据**(文件名/目录/大小/时间戳),**从不读图片内容** ——
>    故 50MB/张 与 5KB/张 的排序成本**完全相同**(`SourceItem` 只含元数据;
>    缩略图走 `ProceduralThumb` 由 `seed` 派生,**零图片 IO**)。
> 2. **排序键已改为每项只算一次**。
>
> **实测发现并修掉的真实浪费**:原实现在**每次比较**里做 `fileName.lowercase()` + `tokenizeNatural()`,
> n=3000 时比较约 **17,254 次**(实测计数)→ 即 1.7 万次字符串小写化与分词,而实际只需 **3000 次**。
> 改为 decorate-sort-undecorate(先为每项预计算排序键一次)后:
>
> | 项目(n=3000) | 优化前 | 优化后 |
> | --- | --- | --- |
> | `sortItems` 单级自然升序 | ~45ms | **~12ms**(3.6×) |
> | `applyRule`(改一次规则,端到端) | ~7.4ms | **~3.7ms**(2×) |
> | `applyRule`(含 300 个固定项) | ~7.1ms | **~4.1ms** |
> | `sortItems` 单级降序 | ~7ms | ~4.6ms |
>
> 即"改一次排序规则"端到端 **2~4ms**,远低于一帧(16ms)。**故"变动即重排"是安全的**。
>
> **等价性守护**:`SortItemsEquivalenceTest` 用 18 条规则 × 含边界样本(补零/前缀零/超长数字/
> 大小写/中文/非法字符/空目录/`created=null`/全等项)的小样本集,把**朴素实现(每次现算)**
> 与**优化实现**逐项比对,确保结果**完全一致**;另测确定性(打乱输入同结果)与不改入参。
>
> **性能护栏的自我修正(值得记录)**:初版护栏用**绝对耗时上界**(n=3000 < 320ms),
> 变异验证发现它**挡不住本次这类回退** —— 退回旧实现(~45ms)**仍在阈值内通过**。
> 修正为**同机相对比较**(朴素实现 vs 优化实现,要求至少快 2×,与机器速度无关),
> 再变异即被捕获。**教训:绝对耗时阈值只能挡灾难性退化;要挡常数倍回退,必须用相对基准。**

> **W3/W4 真机走查与四项缺陷修正(2026-09-17 深夜~09-18 凌晨,用户逐条反馈)**:
>
> **① 三角图标曾"占位但不可见" —— 坐标系陷阱(我照抄官网 SVG 犯的错)**:
> Material Symbols 官网 SVG 用 `viewBox="0 -960 960 960"`(**负 y**),而 Android
> VectorDrawable **没有 viewBox 偏移**语义 —— 我配上 `viewportHeight="960"` + 负坐标后,
> 图形整体落在画布**之外**,表现为"**芯片右侧多出一块占位、却看不到三角**"(用户原话)。
> **正解**:y 加 960 再按 24/960 缩放;换算结果与官方经典 24x24 路径
> (`M7 14l5-5 5 5z` / `M7 10l5 5 5-5z`)**逐点一致**(已独立验证)。
> **为什么 12 项语义测试全绿却没挡住**:`Icon` 节点存在、`testTag` 可查、
> `contentDescription` 正确 —— **只是没有像素**。语义层无法表达"看得见"。
> **补的护栏**:`DirectionIconRenderTest` —— **栅格化后数非透明像素** +
> 断言两三角**朝向相反**;并先拿坏资源验证它**确实红 3 项**(非空转)。
>
> **② 三角偏小(用户:"有点小,可以稍微大点")**:
> 官方 24x24 里三角仅占 **10 宽 x 5 高**,其余全是留白 —— 故 16dp 图标实际只画出
> ~6.7dp 宽。修正:保持 24x24 方形视口(不被非等比拉伸),把**图形放大 1.6 倍**
> (`M7,14 L12,9 L17,14` -> `M4,15.5 L12,7.5 L20,15.5`),图标 16 -> 18dp,
> 实际三角约 **12dp 宽**。
>
> **③ "切换升降序有个滑动卡一下"(用户反馈)**:
> 自动回顶原用 `animateScrollToItem`,在**刚重排过**的列表上逐项动画滚动 ——
> LazyGrid/LazyList 长距离动画滚动需组合中间帧,是 Compose 公认卡顿源,
> 且"对已重排的内容做滚动动画"本身没有意义。改为**瞬时**归位。
> **刻意保留**点按钮时的平滑动画:你主动点击时有意图,动画让空间关系可读。
> **护栏**:`external scroll to top is instant in {grid,list}` —— 阈值来自**实测标定**
> (`scrollToItem` 25 帧 vs `animateScrollToItem` 42 帧,取 32 帧为界),
> 变异验证改回动画即被 2 项捕获。
>
> **④ "仅在顶部切换升降序"时顶部露出上一行碎片(用户反馈,最关键的一条)**:
> **用户补充的范围是本条的决定性线索**:"仅网格视图" + "仅在顶部" + "有一点下滑就不触发"。
> 这条**推翻了我最初"旧滚动偏移中间帧"的解释** —— 在顶部时偏移本就是 0,**没有旧偏移可残留**。
> 真实机制是 **LazyGrid 的 key 锚定**:重排后它按 `key` 记住"原来那个首项",
> 而该项此时已跑到列表后段,于是把**滚动目标改成了它的新下标** ——
> 在顶部表现为视口瞬间被锚定到后段(顶部露出别处的行边);已有下滑时锚定与目标接近,故不明显。
> **修正**:改用 `requestScrollToItem`(在**本次布局内**结算),并**只在 signal 变化时**触发。
> **顺带修掉一个我自己引入的缺陷**:若每次重组都 `requestScrollToItem`,
> 规则变更后用户往下滚会被无关重组**反复拽回顶部** ——
> 该风险由 `after a rule change user can still scroll freely` 守护(变异验证:改回则失败 1 项)。
> **⚠️ 必须如实记录的局限**:本项**在本地建不起回归护栏** ——
> Robolectric 下 `requestScrollToItem` 与 `LaunchedEffect+scrollToItem` 行为**一致**
> (实测都是 25 帧到顶),变异退回旧实现**不会失败**。
> 故本条**只有真机证据**(用户 2026-09-18 实测确认正常),无本地自动护栏。
>
> **⑤ 方法学教训(两次假阴性)**:
> 排查④时我的**真机截图逐像素对比连续给出假阴性** —— 升/降两张截图在规则编辑区报
> "零差异",而用户肉眼明确看到三角且能切换;改用**录屏抽帧**后抓到的"微弱带"
> 经核验是 **H.264 暗场压缩伪影**(偏差仅 2~11),并非真实 UI。
> **结论**:截图/录屏像素对比**不能作为唯一判据**;当它与用户观察冲突时,**用户观察为准**,
> 并应转向代码结构找可解释的机制,而不是继续加码像素分析。
...
**目标**:优先 Android SAF。重点验证最容易推翻技术选型的能力,不先做完整业务。

> ~~再验证 Windows 和 Linux 文件访问~~ —— **2026-09-19 移除**:GUI 只做 Android(§2/§7),§10 明确"桌面端无验收项"。
> 领域模型不得依赖 Windows 专有路径与打包机制的约束(§7 末)继续有效,但它属领域层横向约束,不是本阶段的验证项。
...
- **D2b · 完整平台能力** ⬜ **待办**(D1 收口后启动):
  ...
  - **退出条件**:以下**五项**在目标机型可复现验证,且失败路径有明确的用户可理解提示 ——
    1. 千级缩略图**生成策略**;**2.** **后台任务与进度**(切后台恢复、进度上报);**3.** **授权失效恢复流程**(SecurityException → 引导重选目录);
    **4.** **存储空间预检与不足退路**;**5.** **大归档写入**(Zip64 / 中断恢复 / 目标文件系统上限)。
    > 第 4、5 两项**是正式退出条件,不是加固项**:§10《平台》把它们列为验收标准(「Android 真机完成 **1GB 级**归档的写入验证…更大归档的验收口径为"空间预检 + 明确拒绝 + 引导到电脑"」,并注明依据 §7/§9 D2b)。
    > 原文此处列 5 条却写"上述三项"属**列举不全**的笔误(2026-09-19 校正为"五项"),**不是**"只有三项算数"。
  - ~~Windows / Linux:文件访问与批量预览流程~~ —— **2026-09-19 移除**:与 §7「桌面不再有 GUI 目标,也不再为其投入开发」及 §10「**桌面端无验收项**」直接矛盾,系换栈(2026-09-10)前的遗留条目。领域模型**仍不得依赖 Windows 专有路径与打包机制**(该约束见 §7,继续有效),但这属于领域层的横向约束,不是 D2b 的验证项。

### D3：领域核心与适配层

> **⚠️ 本节性质已于 2026-09-19 重定义(用户确认)**:由"**重新实现领域核心**"改为"**验证与固化**"。
> 动因:D1 的执行注(2026-09-16)把领域逻辑划进 D1,§9.1「切片原则」又要求"各阶段的领域逻辑随该阶段一起进",
> 两者叠加使 **D3 原定四类核心里的三类已由 D1 的 P1–P5 实际交付**。若仍按原字面执行,D1 收口后会**重复造已完成的领域层**。

**原目标(保留为意图,不再作为"从零实现"的要求)**:将排序、命名、元数据、ComicInfo.xml、CBZ 生成、计划和报告拆成可测试核心；平台层只负责资源访问、缩略图、权限和任务生命周期。

**现状对照(2026-09-19)** —— 已交付部分**不重复实现,直接承接**:

| 原四类核心 | 现状 | 证据 |
| --- | --- | --- |
| 排序 | ✅ **已交付** | `pixfold-d1/domain/sort/`(自然排序、多级排序、`PageOrderState`/`moveItemTo`/`applyRule`/图钉) |
| 命名 | ✅ **已交付** | `domain/naming/`(求值九步、冲突五类、改名计划)+ `domain/model/Naming.kt` |
| 元数据 | ✅ **已交付** | `domain/comic/`(建议值+来源、ComicInfo.xml、页码重编号、批量设置逐项例外) |
| **打包** | ⬜ **未交付** | D1 是交互原型、**不连真实文件系统**(§9 D1 执行注),六项交付里没有打包;P5 只做 XML **文本预览** |

另:`domain/` 为**纯 Kotlin JVM 模块,Gradle 依赖边界强制其不依赖 UI 与平台层** —— 这正是 D3 要的"可测试核心 + 平台层隔离"的结构前提,已由 P1 落地。

**重定义后的退出条件(可判定)**:

1. **打包核心**落地并单测通过:CBZ 生成/写入计划(`PackagePlan`/`OperationPlan` 语义,§5)、执行报告(`ExecutionReport`)、撤销记录(`UndoRecord`)—— 其中计划与报告**在 D1 的 P6 已有交互原型**,D3 需确认为"可在正式工程承接的领域核心"而非仅 UI 内的实现;
2. **适配层抽取确认**:`domain/` 保持零 Android 依赖,平台层(SAF/ContentResolver)只承担资源访问、缩略图、权限、任务生命周期 —— 依赖边界须有可执行检查(如 Gradle 依赖断言),而非仅靠自觉;
3. **四类核心的测试基线连续**:`domain/` 既有 143 项单测在迁入正式工程后**全部保留并通过**(不以"重写"替代,避免语义漂移)。

> **归属澄清**:原先"打包"既写在 D3、又落在 D2b(大归档写入)与 D4(MVP 端到端)里,三处边界模糊。
> 2026-09-19 明确:**打包的领域核心(生成 CBZ、计划、报告、撤销)→ D3**;**打包的平台能力(Zip64/中断/空间预检)→ D2b**;**真实文件端到端跑通 → D4**。

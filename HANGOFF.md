...
>
> ⚠️ **待确认(2026-09-10)**：清单原为「Windows | Android」双列，而 GUI 已只做 Android，**Windows 列已无实际意义**。按 AGENTS.md「清单条目变更须用户确认」，未擅自删列。待定：① 保留双列（继续双端走查，代价是与新范围不符）② 删 Windows 列、20 项改为 Android 单端。

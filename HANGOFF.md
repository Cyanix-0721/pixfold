...
> 项目名：**PixFold**
> 状态：**设计阶段**（2026-09-03 立项；2026-09-08 SAF 可行性 spike 5/5 验收、技术栈锁定，当前主线 D1 交互原型）
> 目标平台：**Windows、Android（优先）、Linux**
> 技术栈：**Flutter + Dart（2026-09-08 锁定，SAF 可行性 spike 5/5 验收，见 §8.1）**
...
- 技术栈已锁定 **Flutter + Dart**(2026-09-08,SAF 可行性 spike 5/5 验收通过,§8.1);对照 CMP 仅当 §8.2 证伪条件 ③ 被 D1 证实且修复成本不可接受时才启用。
...
**排序键定位与字段序(2026-09-10 用户确认)**:可用排序键 = 文件名(自然) / 文件名(字典) / 目录名 / 修改时间 / 创建时间 / 文件大小,下拉即按此顺序。**自然序是主路径**——用户以 `IMG_1_xxx` 这类规范命名在源头保证顺序,工具不为 `IMG_10` 这类歧义命名兜底;**字典序保留**(与外部工具/脚本输出对齐、等宽编码、混排字符集排查时作对照),紧随自然序之后;时间与大小次常用。多级排序最多 4 级(D1 实现)。
...
> **✅ 已锁定:Flutter + Dart**。SAF 可行性 spike(§8.3)5/5 验收通过,证伪条件 ①② 均已排除(Android 16 真机,一加 Ace 3 Pro);桌面交互风险(D1)按 §8.2 判定为"已知可做",不再构成换栈理由。CMP 对照、Tauri 本轮不启用;若 D1 桌面表格/拖拽触发证伪条件 ③ 且修复成本不可接受,再重审 CMP。
...
### 8.3 SAF 可行性 spike 执行方案(2026-09-07 记录,环境验收后启动)

**状态**(2026-09-08):✅ **SAF 可行性 spike 验收全部通过,证伪条件 ①② 排除,正式锁栈 Flutter**。工程建于 `C:\Personal\pixfold\pixfold-saf-spike`(2026-09-10 起随验证工程提交至本仓库 ready 分支,验证阶段结束后按需迁移/清理),一加 Ace 3 Pro(Android 16/API 36)无线 adb 真机验收。**验收清单 5/5 全过**:
...
**收束**(2026-09-08):验收清单 5/5 全过,无"待补验"项;证伪条件 ①② 排除,Flutter 已锁定(§8.1)。**技术栈决策至此收束**(注意:本 spike 属 §8 技术栈决策流程,不是 §9 的 D2 阶段,详见 §9 归属注),可启动 D1 交互原型(栈已锁,原型资产不浪费)。spike 工程 `C:\Personal\pixfold\pixfold-saf-spike` 留作 Android SAF 通道参考实现(正式工程可直接迁移 Kotlin channel 代码)。

**目标**:最小工程验证 Android SAF 全闭环,用于判定 §8.2 证伪条件 ①②;不做任何业务 UI。

**工程形态**:

- 目录:`C:\Personal\pixfold\pixfold-saf-spike`(2026-09-10 起入库 ready 分支;验证阶段资产,非正式工程);
...
| D2 平台能力验证         | ⬜ 待办                 | 缩略图策略 / 后台任务与进度 / 授权失效恢复,三项真机可复现且有失败路径提示 |
...
> D1 执行注(2026-09-09):D1 原型工程已建 **`C:\Personal\pixfold\pixfold-d1`**(2026-09-10 起随验证工程提交至本仓库 ready 分支,与 `pixfold-saf-spike/` 同级;不连真实文件系统,确定性 mock 数据)。范围 = 本节 D1 六项交付;验收平台 **Windows + Android 双端**(用户 2026-09-09 决策);拖拽/表格实现策略 = **内置优先**(曾引入 `reorderable_grid_view 2.2.8` 做对照,2026-09-10 实测其桌面端 drop 不落地,已弃用,详见 §12.5;**当前零三方依赖,拖拽为自研**)。领域逻辑(自然排序/多级排序/命名求值/冲突检测/ComicInfo.xml/页码重编号)以可测试纯 Dart 实现,当前 **16 项测试**(3 项拖拽手势回归 + 1 项 UI 重建回归 + 2 项固定位置)。
...
> 走查修复注(2026-09-10 晚):用户走查反馈 ④ **网格模式缺图钉操作入口**——原 `ThumbCard` 收了 `onPin` 却只在"已固定"时画一个静态角标,没有任何可点入口(列表行有自己的 `IconButton`,故只有网格缺)。修复:网格卡片右下角改为可点圆形按钮(未固定 = 空心图钉 / 已固定 = 实心琥珀,带 tooltip,26px 热区),点击调 `togglePin`,与列表入口行为一致。领域层固定语义本就完整(`applyRule` 把固定项钉在原 index 并清空人工标记),补 2 项测试钉死:逻辑(固定 → 换排序规则仍在原位;解除 → 回到规则决定的位置)+ UI(点空心图钉 → 变实心)。当前 test **16/16**。**教训:UI 组件的可选回调"传了但没被使用"是静默缺口,类型检查与常规测试都发现不了——只能靠走查或按验收清单逐项核对**(此项正对应清单第 6 项)。
> 走查修复注(2026-09-10 晚②):用户走查反馈 ⑤ **排序字段下拉顺序不合理**——原顺序把"文件名(字典)"排在"文件名(自然)"之前,与主路径不符。按用户定序调整为 `文件名(自然) → 文件名(字典) → 目录名 → 修改时间 → 创建时间 → 文件大小`(枚举声明顺序即下拉顺序);默认规则 `[目录名↑, 文件名(自然)↑]` 与多级排序(≤4 级)逻辑不变。定序依据与排序键定位见 §4。
...
> **代码侧自测进度(2026-09-10 晚,供走查参照)**:第 20 项(工程健康)已过——`analyze` 零告警、`test 16/16`;第 3/4/5/6/16/17 项的逻辑语义已有测试钉死(`moveItemTo` 落位、固定位置在重排后保持、危险项默认关闭),真机只需确认"点得中、看得见";第 1/2/7/8/9/10/11/12/13/14/15/18/19 项为交互与观感,需逐条实际操作核对。
...
**spike 的归属(2026-09-10 修正)**:SAF 可行性 spike 属 **§8 技术栈决策流程**(§8.2 判定 / §8.3 执行),回答的是"**能不能做**"(失败则换栈),**不是本节 D2 阶段的一部分**;本节 D2 回答的是"**做出来是否达产品要求**"。该 spike 按 §8.2 顺序调整提前于 D1 执行,2026-09-08 真机 5/5 通过后归档。此前把它并入 D2 写作"D2a/D2b"不严谨,已修正(用户提出)。
...
11. **自然排序是否需要支持中文数字**：当前自然排序只做"数字段 vs 字符段"切分，**不识别中文数字**（`第一话` / `第十话` 会按汉字逐字比，`第十话` 可能排在 `第二话` 前）。实际遇到中文数字命名时需加归一化。（2026-09-10 记录）
12. **命名建议的"序号"取哪个**：现取"第 1 步整理后的当前顺序"（拖拽会改变序号）；是否需要提供"沿用原文件名解析出的原始序号"（如 `IMG_1_xxx` 里的 `1`）作为可选项？直接决定"拖拽后序号是否变"。（2026-09-10 记录）

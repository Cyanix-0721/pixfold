...
| **P3** | 拖拽排序(网格+列表,松手落地)、图钉入口、排序规则编辑器(多级/批次+本组独立) ✅ **完成(2026-09-17)** | 3, 4, 5, 6, 7 | 1+2+3 |
...
**P3 完成记录(2026-09-17)**:拖拽排序(网格 + 列表)、图钉入口、人工调整标记与一键重置、多级排序规则编辑器交付。
**验证实况**:`:domain:test` **39 项**、`:app:testDebugUnitTest` **55 项**、`assembleDebug` 成功、`lintDebug` **零告警**。
新增领域层 `WorkflowAState`(批次默认 `batchRule` + `customRuleCollections`,实现"批次统一 + 逐项例外",验收第 7 项)。
